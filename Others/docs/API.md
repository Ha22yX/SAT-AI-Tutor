...
| `POST /api/auth/admin/create` | ✅ (root) | `{"email":"tutor@example.com","username":"tutor1","password":"StrongPass123!"}` | Only the root admin may call; override `ROOT_ADMIN_USERNAME` and `ROOT_ADMIN_PASSWORD` in `.env` for real deployments. |
