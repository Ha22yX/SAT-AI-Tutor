...
SAT AI Tutor 是一个面向 SAT 练习和复盘的全栈学习平台，包含自适应练习、每日学习计划、讲解播放、PDF 题目导入、学习数据分析和后台题库管理。

项目重点是把做完的题目变成可复盘的学习材料。讲解可以包含分步推理、证据高亮、板书提示、图像引用和中英文输出，帮助学生理解刚做错的题，也给管理员提供导入和审核 SAT 风格题目的流程。

## 项目重点

- SAT 风格讲解，而不是只给简短答案。
- 可视化讲解格式，用于标记 passage、题干、选项和图表。
- 数学复盘覆盖画图、代入、代数、估算和自检等路径。
- 阅读与写作复盘关注证据、语法逻辑和干扰项。
- 自适应练习会参考掌握度、最近表现、复习间隔和学习计划。
- 后台支持 PDF 导入、草稿审查、图像裁剪、题目验证和发布。
- 支持英文和中文界面/讲解。
