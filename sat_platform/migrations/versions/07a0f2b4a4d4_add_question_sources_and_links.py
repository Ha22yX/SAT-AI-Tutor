...
def upgrade():
    ...
    existing_indexes = (
        {idx["name"] for idx in inspector.get_indexes("question_sources")}
    ...
    if "source_id" not in job_columns:
        op.add_column(
            "question_import_jobs", sa.Column("source_id", sa.Integer(), nullable=True)
        )
    existing_indexes = {
        idx["name"] for idx in inspector.get_indexes("question_import_jobs")
    }
    ...
    if "source_id" not in draft_columns:
        op.add_column(
            "question_drafts", sa.Column("source_id", sa.Integer(), nullable=True)
        )
...
def downgrade():
    ...
    if (
        ...
        and op.f("fk_questions_source_id_question_sources") in question_fks
    ):
        op.drop_constraint(
            op.f("fk_questions_source_id_question_sources"),
            "questions",
            type_="foreignkey",
        )
    ...
    draft_fks = {fk["name"] for fk in inspector.get_foreign_keys("question_drafts")}
    draft_index_names = {
        idx["name"] for idx in inspector.get_indexes("question_drafts")
    }
    draft_columns = [col["name"] for col in inspector.get_columns("question_drafts")]
    if (
        bind.dialect.name != "sqlite"
        and op.f("fk_question_drafts_source_id_question_sources") in draft_fks
    ):
        op.drop_constraint(
            op.f("fk_question_drafts_source_id_question_sources"),
            "question_drafts",
            type_="foreignkey",
        )
    if op.f("ix_question_drafts_source_id") in draft_index_names:
        op.drop_index(
            op.f("ix_question_drafts_source_id"), table_name="question_drafts"
        )
    if "source_id" in draft_columns:
    ...
    if (
        bind.dialect.name != "sqlite"
        and op.f("fk_question_import_jobs_source_id_question_sources") in job_fks
    ):
        op.drop_constraint(
            op.f("fk_question_import_jobs_source_id_question_sources"),
            "question_import_jobs",
            type_="foreignkey",
        )
    if op.f("ix_question_import_jobs_source_id") in job_index_names:
        op.drop_index(
            op.f("ix_question_import_jobs_source_id"), table_name="question_import_jobs"
        )
    ...
    if op.f("ix_question_sources_uploaded_by") in (
        ...
        op.drop_index(
            op.f("ix_question_sources_uploaded_by"), table_name="question_sources"
        )
    if "question_sources" in table_names:
