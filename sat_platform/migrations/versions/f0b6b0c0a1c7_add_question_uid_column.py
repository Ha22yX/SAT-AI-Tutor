...
def _ensure_column():
    ...
    with op.batch_alter_table("questions") as batch_op:
        batch_op.add_column(
            sa.Column("question_uid", sa.String(length=32), nullable=True)
        )
...
def upgrade():
    ...
    with op.batch_alter_table("questions") as batch_op:
        batch_op.alter_column(
            "question_uid", existing_type=sa.String(length=32), nullable=False
        )
        existing_uniques = {
            uc["name"] for uc in inspector.get_unique_constraints("questions")
        }
        if "uq_questions_question_uid" not in existing_uniques:
            batch_op.create_unique_constraint(
                "uq_questions_question_uid", ["question_uid"]
            )
    ...
    if "ix_questions_question_uid" not in existing_indexes:
        op.create_index(
            "ix_questions_question_uid", "questions", ["question_uid"], unique=False
        )
