...
def downgrade():
    ...
    if "study_plan_tasks" in inspector.get_table_names():
        existing_indexes = {
            idx["name"] for idx in inspector.get_indexes("study_plan_tasks")
        }
        if "ix_study_plan_tasks_user_date" in existing_indexes:
            op.drop_index(
                "ix_study_plan_tasks_user_date", table_name="study_plan_tasks"
            )
    ...
    if "plan_block_id" in session_columns:
        ...
        if op.f("ix_study_sessions_plan_block_id") in existing_session_indexes:
            op.drop_index(
                op.f("ix_study_sessions_plan_block_id"), table_name="study_sessions"
            )
