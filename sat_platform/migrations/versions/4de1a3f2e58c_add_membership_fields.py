...
def upgrade():
    with op.batch_alter_table("users") as batch_op:
        batch_op.add_column(
            sa.Column(
                "membership_expires_at", sa.DateTime(timezone=True), nullable=True
            )
        )
        batch_op.add_column(
            sa.Column("ai_explain_quota_date", sa.Date(), nullable=True)
        )
        batch_op.add_column(
            sa.Column(
                "ai_explain_quota_used",
                sa.Integer(),
                nullable=False,
                server_default="0",
            )
...
def downgrade():
    op.drop_index(
        op.f("ix_user_subscription_logs_user_id"), table_name="user_subscription_logs"
    )
    op.drop_table("user_subscription_logs")
    with op.batch_alter_table("users") as batch_op:
        for column in (
            "membership_expires_at",
            "ai_explain_quota_date",
            "ai_explain_quota_used",
        ):
