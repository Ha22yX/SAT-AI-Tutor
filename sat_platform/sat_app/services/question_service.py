...
def list_questions(
    ...
    query = Question.query.options(
        joinedload(Question.passage), joinedload(Question.source)
    )
    if section:
    ...
    return query.order_by(Question.created_at.desc()).paginate(
...
def create_question(payload: dict, *, commit: bool = True) -> Question:
    ...
    payload.pop("choice_figure_keys", None)
...
def delete_question(question: Question, *, commit: bool = True) -> None:
    ...
    UserQuestionLog.query.filter_by(question_id=question.id).delete(
        synchronize_session=False
    )
    QuestionReview.query.filter_by(question_id=question.id).delete(
        synchronize_session=False
    )
    QuestionExplanationCache.query.filter_by(question_id=question.id).delete(
        synchronize_session=False
    )
    QuestionFigure.query.filter_by(question_id=question.id).delete(
        synchronize_session=False
    )
