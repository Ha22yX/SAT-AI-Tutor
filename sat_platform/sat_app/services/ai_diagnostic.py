...
def _build_prompt(
    user: User, predictor_payload: dict, progress: list[dict]
) -> list[dict[str, str]]:
    system_prompt = (
    ...
    user_prompt = json.dumps(
        {
            "student": {
                "email": user.email,
                "level": user.profile.language_preference if user.profile else "en",
            },
...
def _fallback_narrative(predictor_payload: dict, progress: list[dict]) -> dict:
    ...
    return {
        ...
        "risk_factors": [
            ...
            (
                f"Recent accuracy: {latest_accuracy:.2f}"
...
def _store_report(
    user_id: int, predictor_payload: dict, narrative: dict
) -> DiagnosticReport:
