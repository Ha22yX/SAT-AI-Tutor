...
class QuestionImportJob(db.Model):
    ...
    source_id = db.Column(
        db.Integer, db.ForeignKey("question_sources.id"), nullable=True, index=True
    )
...
class QuestionDraft(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    job_id = db.Column(
        db.Integer, db.ForeignKey("question_import_jobs.id"), nullable=False, index=True
    )
    source_id = db.Column(
        db.Integer, db.ForeignKey("question_sources.id"), nullable=True, index=True
    )
    ...
    job = db.relationship(
        "QuestionImportJob",
        backref=db.backref(
            "drafts", cascade="all, delete-orphan", passive_deletes=True
        ),
    )
    ...
    def serialize(self) -> dict:
        ...
        return {
            ...
            "requires_figure": bool(
                payload.get("has_figure") or payload.get("choice_figure_keys")
            ),
